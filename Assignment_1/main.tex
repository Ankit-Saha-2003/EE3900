...
\begin{document}
	...
	For the $x(n)$ given in \eqref{eq:filter_input}
	\begin{align}
		X(z) &= \z{x(n)} \\
		&= \sum _{n=0}^{5}x(n)z^{-n} \\
	...
	Therefore, the system is stable. The same has been verified in the following code
	\begin{lstlisting}
	...
	Alternatively, download the following Python code for computing the convolution by using a Toeplitz matrix which is generated by a C code
	\begin{lstlisting}
		wget https://github.com/Ankit-Saha-2003/EE3900/raw/main/Assignment_1/codes/5.5.c
...
IFFT.
	...
	Download the following Python code that plots Fig. \ref{fig-6.5}
	\begin{lstlisting}
	...
	\begin{equation}
		\sum _{m=0}^{M}a\brak{m} z^{-m} Y(z)	 =\sum _{k=0}^{N}b\brak{k}x\brak{n-k}
	\end{equation}
